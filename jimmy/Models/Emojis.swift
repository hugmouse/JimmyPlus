...
class Emojis {
  ...
  init() {
    if let data = UserDefaults.standard.data(forKey: "emojis"),
    ...
    if let data = UserDefaults.standard.data(forKey: "officialemojis"),
      let decoded = try? JSONDecoder().decode([Emoji].self, from: data)
    {
      ...
      if let message = message {
        if let range = message.firstRange(of: Data("\r\n".utf8)) {
          ...
          if header.code == 20 && header.contentType.starts(with: "text/plain") {
            let emo = String(decoding: contentData, as: UTF8.self).trimmingCharacters(
