...
extension String {
  ...
  public var encodedURLString: String? {
    ...
    pathAndQuery =
      pathAndQuery.addingPercentEncoding(withAllowedCharacters: allowedCharacters) ?? ""

    var result = "\(urlParts.scheme)\(urlParts.delim)"

    if let username = urlParts.username?.addingPercentEncoding(
      withAllowedCharacters: .urlUserAllowed)
    {
      if let password = urlParts.password?.addingPercentEncoding(
    ...
    if var fragment = urlParts.fragment {
      ...
      fragment =
        fragment.addingPercentEncoding(withAllowedCharacters: fragmentAlloweCharacters) ?? ""
  ...
  public var decodedURLString: String? {
    let urlParts = self.urlParts
    ...
    var result =
      "\(urlParts.scheme)\(urlParts.delim)\(usernamePassword)\(host)\(urlParts.pathAndQuery.removingPercentEncoding ?? "")"
...
extension URL {
  ...
  public init?(unicodeString: String) {
  ...
  public init?(unicodeString: String, relativeTo url: URL?) {
    ...
    if !parts.host.isEmpty {
      ...
      allowedCharacters.insert(charactersIn: "%?#")
      guard
        let encoded = unicodeString.addingPercentEncoding(withAllowedCharacters: allowedCharacters)
      else { return nil }
...
extension StringProtocol {
  ...
  fileprivate var punycodeEncoded: String? {
    var result = ""
    ...
    while h < inputLength {
      ...
      for c in scalars {
        ...
        if c.value == n {
          ...
          while true {
            let t = k <= bias ? Punycode.tmin : k >= bias + Punycode.tmax ? Punycode.tmax : k - bias
  ...
  /// Returns `nil` on error.
  /// - Todo: Throw errors on failure instead of returning `nil`.
  fileprivate var punycodeDecoded: String? {
    var result = ""
    let scalars = self.unicodeScalars
...
extension String {
  ...
  fileprivate var urlParts: URLParts {
    ...
    return URLParts(
      scheme: scheme, delim: delim, host: host, pathAndQuery: path, username: username,
      password: password, fragment: fragment)
  }

  fileprivate enum UTS46MapError: Error {
  ...
  /// - Throws: `UTS46Error`.
  fileprivate func mapUTS46() throws -> String {
  ...
  fileprivate var isValidLabel: Bool {
    guard
      self.precomposedStringWithCanonicalMapping.unicodeScalars.elementsEqual(self.unicodeScalars)
    else { return false }

    guard (try? self.mapUTS46()) != nil else { return false }

    if let category = self.unicodeScalars.first?.properties.generalCategory {
      if category == .nonspacingMark || category == .spacingMark || category == .enclosingMark {
        return false
      }
    }
  ...
  fileprivate var hasValidJoiners: Bool {
    try! UTS46.loadIfNecessary()
...
private enum Punycode {
  ...
  static func decodeDigit(_ cp: UInt32) -> UInt32 {
    return cp &- 48 < 10
      ? cp &- 22 : cp &- 65 < 26 ? cp &- 65 : cp &- 97 < 26 ? cp &- 97 : Self.base
  }
