...
class ContentParser {
    ...
    if let range = content.firstRange(of: Data("\r\n".utf8)) {
      ...
      if (20...29).contains(self.header.code) {
        ...
        if self.header.contentType.starts(with: "image/") {
          ...
          self.attrStr = parseGemText(
            String(decoding: contentData, as: UTF8.self).replacingOccurrences(of: "\r", with: ""))

        } else if self.header.contentType.starts(with: "text/") {
          self.attrStr = NSMutableAttributedString(
            string: String(decoding: contentData, as: UTF8.self),
            attributes: self.getAttributesForType(.pre, link: nil))
  ...
  func parseGemText(_ content: String) -> NSAttributedString {
    ...
    for (index, l) in lines.enumerated() {
      ...
      let nextBlockType: BlockType =
        index + 1 < lines.count ? getBlockType(String(lines[index + 1])) : .end

      if (blockType != nextBlockType) || blockType == .link || blockType == .title1
        || blockType == .title2 || blockType == .title3
      {
        ...
        if (blockType == .title1 || blockType == .title1 || blockType == .title3)
          && firstTitle == nil
        {
          firstTitle = str.replacingOccurrences(of: "#", with: "").trimmingCharacters(
        ...
        if blockType == .link {
          ...
          result.append(
            NSAttributedString(string: str.trimmingCharacters(in: .whitespaces), attributes: attr))
        }
  ...
  func getBlockType(_ l: String) -> BlockType {
    let line = l.trimmingCharacters(in: .whitespacesAndNewlines).trimmingCharacters(
  ...
  func getLinkAS(_ str: String) -> NSAttributedString {
    ...
    var newStr = str.replacingOccurrences(
      of: "=>", with: "", options: [], range: .init(NSRange(location: 0, length: 2), in: str)
    ...
    image1String.addAttribute(
      .foregroundColor, value: NSColor.secondaryLabelColor, range: NSRange(location: 0, length: 2))
    image1String.addAttribute(
      .font, value: NSFont.systemFont(ofSize: tab.fontSize * 1.4),
      range: NSRange(location: 0, length: 2))
    image1String.addAttribute(
      .baselineOffset, value: -tab.fontSize * 0.1, range: NSRange(location: 0, length: 2))
    image1String.addAttribute(
      .paragraphStyle, value: attr[.paragraphStyle] ?? [], range: NSRange(location: 0, length: 2))
  ...
  func getQuoteAS(_ str: String) -> NSAttributedString {
    // create our NSTextAttachment
    let image1Attachment = NSTextAttachment()
    let newStr = str.replacingOccurrences(
      of: ">", with: "", options: [], range: .init(NSRange(location: 0, length: 2), in: str)
    ...
    image1String.addAttribute(
      .foregroundColor,
      value: NSColor.controlAccentColor.blended(withFraction: 0.3, of: NSColor.green)
        ?? NSColor.green, range: range)
    image1String.addAttribute(
      .font, value: NSFont.systemFont(ofSize: tab.fontSize * 1.4), range: range)
    ...
    let pst: NSMutableParagraphStyle =
      attr[.paragraphStyle] as? NSMutableParagraphStyle ?? NSMutableParagraphStyle()
  ...
  func getLineForType(_ str: String, type: BlockType) -> String {
    ...
    case .list:
      return str.replacingOccurrences(
        of: "*", with: "•", options: [], range: .init(NSRange(location: 0, length: 1), in: str)
      ).trimmingCharacters(in: .whitespaces)
    ...
    case .title3:
      return str.replacingOccurrences(
        of: "#", with: "", options: [], range: .init(NSRange(location: 0, length: 3), in: str)
      ).trimmingCharacters(in: .whitespaces)
    case .quote:
      return str.replacingOccurrences(
        of: ">", with: "", options: [], range: .init(NSRange(location: 0, length: 1), in: str)
  ...
  var preStyle: [NSAttributedString.Key: Any] {
    ...
    pst.tabStops = (1...20).map {
      NSTextTab(textAlignment: .left, location: CGFloat($0) * tabInterval)
    }
  ...
  private func parseLink(_ l: String) -> Link {
    let line = l.replacingOccurrences(of: "=>", with: "").trimmingCharacters(
      in: .whitespacesAndNewlines)
